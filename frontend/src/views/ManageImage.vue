        ...
        <form role="form">
          <div>
            <v-container grid-list-md>
              <base-input v-model="editedItem.analysisId" type="hidden"></base-input>
              <base-input v-model="editedItem.attributes" type="hidden"></base-input>
              <div v-for="(attribute, id) in attributes" :key="id">
                {{attribute.name}}
                <base-input class="mb-3" v-model="editedItem.values[id]"></base-input>
              </div>
            </v-container>
          </div>
          <template>
...
export default {
  ...
  methods: {
    ...
    initialize() {
      axios
        .get(
          `http://localhost:3000/api/analysis/management/image/` +
            this.analysisId
        )
        .then(response => {
    ...
    setAttribute() {
      axios
        .get(
          `http://localhost:3000/api/analysis/management/image/attribute/` +
            this.analysisId
        )
        .then(response => {
    ...
    viewImage(item) {
      ...
      axios
        .get(
          `http://localhost:3000/api/analysis/management/image/value/` + item.id
        )
        .then(response => {
    ...
    deleteItem(item) {
      if (confirm("Are you sure you want to delete?")) {
        axios
          .delete(
            `http://localhost:3000/api/analysis/management/image/` + item.id,
            { data: { path: item.path } }
          )
          .then(response => {
    ...
    deleteSelectedItem(item) {
      if (confirm("Are you sure you want to delete?")) {
        axios
          .delete(`http://localhost:3000/api/analysis/management/image`, {
            data: { images: item }
          })
          .then(response => {
            for (let i of item) {
    ...
    save() {
      if (confirm("Are you sure you want to create this item?")) {
        ...
        axios
          .post("http://localhost:3000/api/analysis/image/upload/", formData, {
            headers: {
          ...
          .then(response => {
            axios
              .post(`http://localhost:3000/api/analysis/management/image`, {
                image: item,
                fileName: fileName
              })
              .then(response => {
